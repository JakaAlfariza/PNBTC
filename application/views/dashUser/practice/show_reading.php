                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.status === 'continue') {
                            ...
                            // Handle the end of the test or no more sections
                             if (confirm('This is the last question, submit all?')) {
                                clearInterval(timerInterval);
                                localStorage.removeItem('timeLeft');
                                window.location.href = '<?= base_url("cpractice/complete") ?>';
                            }else {
                                // Handle cancellation or redirect to previous page if necessary
                                window.history.back();
                            }
                        }
                    } catch (e) {
